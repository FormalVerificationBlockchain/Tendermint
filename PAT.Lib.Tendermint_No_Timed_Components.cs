...
namespace PAT.Lib {
    ...
    public class MessageLog : ExpressionValue {
        ...
        public void AddPrevote(PrevoteMessage prevote) {
            if (this.prevotes[prevote.round] == null) {
        ...
        public void AddPrecommit(PrecommitMessage precommit) {
            if (this.precommits[precommit.round] == null) {
        ...
        public bool ContainsProposal(int round, int validRound) {
            return this.proposals[round] != null && 
                    (validRound == -2 || this.proposals[round].validRound == validRound);
        ...
        public bool ContainsSufficientPrevotes(int round, int value) {
            if (value == -2) {
                return this.prevotes[round] != null && this.prevotes[round].Count >= 3;
            } else {
                return this.prevotes[round] != null && this.prevotes[round].Where(prevote => prevote.value == value).ToList().Count >= 3;
            }
        ...
        public bool ContainsAllPrevotesWithoutMajorityValue(int round) {
            return this.prevotes[round] != null && this.prevotes[round].Count == 4 && 
                    !this.ContainsSufficientPrevotes(round, 0) && !this.ContainsSufficientPrevotes(round, 1);
        }


        public bool ContainsSufficientPrecommits(int round, int value) {
            if (value == -2) {
                return this.precommits[round] != null && this.precommits[round].Count >= 3;
            } else {
                return this.precommits[round] != null && this.precommits[round].Where(precommit => precommit.value == value).ToList().Count >= 3;
            }
            // if (value == MessageLog.DefaultValue) {
        ...
        public bool ContainsPrecommitsWithoutMajorityValue(int round) {
            return this.precommits[round] != null && this.precommits[round].Count == 4 && 
                    !this.ContainsSufficientPrecommits(round, 0) && !this.ContainsSufficientPrecommits(round, 1);
        }

        public bool ContainsProposalAndSufficientPrevotesForPrevoting(int round) {
            return this.proposals[round] != null && 
                    this.proposals[round].validRound >= 0 && 
                    this.proposals[round].validRound < round && 
                    this.ContainsSufficientPrevotes(this.proposals[round].validRound, this.proposals[round].value);
            // return this.proposals.ContainsKey(round) && 
        ...
        public bool ContainsProposalAndSufficientPrevotesForPrecommitting(int round) {
            return this.proposals[round] != null && 
                    this.ContainsSufficientPrevotes(round, this.proposals[round].value);
            // return this.proposals.ContainsKey(round) && 
        ...
        public bool ContainsProposalAndSufficientPrecommits() {
            for (int i = 0; i < this.proposals.Length; i++) {
                if (this.proposals[i] != null && this.ContainsSufficientPrecommits(i, this.proposals[i].value)) {
                    return true;
                }
            }
            return false;
        ...
        public bool ContainsProposalAndSufficientAdversePrecommits(int round) {
            return this.proposals[round] != null && 
                    this.precommits[round] != null && 
                    this.precommits[round].Where(precommit => precommit.value != this.proposals[round].value).ToList().Count > 1;
            // return this.proposals.ContainsKey(round) && 
        ...
        public int GetLatestRoundWithSufficientMessages() {
            for (int i = this.proposals.Length - 1; i >= 0; i--) {
                if ((this.proposals[i] == null ? 0 : 1) + (this.prevotes[i] == null ? 0 : this.prevotes[i].Count) + (this.precommits[i] == null ? 0 : this.precommits[i].Count) > 1) {
                    return i;
                }
            }
            return -1;
        ...
        public override string ExpressionID {
            get {
                string returnString = "";
                for (int i = 0; i < this.proposals.Length; i++) {
                    if (this.proposals[i] != null) {
                        returnString += this.proposals[i].ToString() + "; ";
                    }
                    if (this.prevotes[i] != null) {
                        returnString += String.Join("; ", this.prevotes[i].Select(prevote => prevote.ToString()).ToArray()) + "; ";
                    }
                    if (this.precommits[i] != null) {
                        returnString += String.Join("; ", this.precommits[i].Select(precommit => precommit.ToString()).ToArray()) + "; ";
                    }
                }
                return returnString;
